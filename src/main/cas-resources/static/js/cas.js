...
function disableEmptyInputFormSubmission() {
    var fields = $('#fm1 input[name="password"],[name="phoneCode"],[name="emailCode"]');

    if (fields.length == 3) {
        fields.on('input', function (event) {
            var enableSubmission = $('#fm1 input[name="password"]').val().trim() ||
                $('#fm1 input[name="phoneCode"]').val().trim() ||
                $('#fm1 input[name="emailCode"]').val().trim();
